...
const apiUrl = 'https://mobile.rumkitpelamonia.id/pasien-rest/'; // API Server URL
...
var app  = new Framework7({
  ...
  id: 'id.pelamonia.register', // App bundle ID
  name: 'Pelamonia', // App name
  dialog: {
    title: 'Peringatan',
    buttonOk: 'Ya',
    buttonCancel: 'Tidak'
...
if (debug == '1') {
  ...
  function appReady(){
    ...
    document.addEventListener('resume', function(){
      ga('send', 'pageview' , {'location' : '36.66.243.186/pasien-rest/index.html' });
    }, false);
  ...
  app.request.post(apiUrl + 'api.php', {
    ...
    if(data.state == "notifberkas") {
      ...
      app.notification.create({
        ...
        titleRightText: 'saat ini',
        subtitle: 'RS Pelamonia',
        text: '<b>'+ stts +'</b>. Silahkan menuju ke Klinik pilihan anda di RS Pelamonia untuk mendapatkan layanan prioritas tanpa harus antri di loket pendaftaran.',
        closeButton: true,
        closeOnClick: true,
        on: {
          close: function () {
        ...
        app.request.post(apiUrl + 'api.php', {
          ...
          if(data.state == "notfound") {
            ...
            for(i=0; i<data.length; i++) {
              ...
              html += '    <div style="font-size:12px;">' + data[i]['jam_mulai'] + ' s/d ' + data[i]['jam_selesai'] + ' WIT</div>';
              html += '   </div>';
              html += '  </div>';
              html += ' </div>';
              html += '</li>';
